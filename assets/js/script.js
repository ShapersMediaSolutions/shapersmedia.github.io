            ...
            function headerStyle() {
                if($('.main-header').length){
                    ...
                    var headerLower = siteHeader.find('.header-lower');
                    var fadeEnd = Math.max(120, $(window).height() * 0.15);
                    var opacity = 0;
                    
                    if (windowpos >= fadeEnd) {
                        opacity = 0.7;
                    } else if (windowpos > 0) {
                        opacity = (windowpos / fadeEnd) * 0.7;
                    }

                    if (headerLower.length) {
                        headerLower.css('background-color', 'rgba(0,0,0,' + opacity + ')');
                    } else {
                        siteHeader.css('background-color', 'rgba(0,0,0,' + opacity + ')');
                    }
